...
function make_panel(opts)
  ...
  -- Title (centred horizontally, anchored top)
  local title_e = nil
  if title_text ~= "" then
    -- make_label already calls add_text once — do NOT call add_text again
    title_e = make_label(cx, cy - ph/2 + 14, title_text, title_size,
  ...
  for i, item in ipairs(items) do
    ...
    local e = make_label(cx, items_start_y + (i - 1) * item_gap,
                         item.text, isize, icol[1], icol[2], icol[3])
...
function make_pause_overlay(cx, top_y)
  ...
  -- Hint label below the panel
  local hint_e = make_label(cx, top_y + ph - 22,
                             "UP/DOWN  ENTER  or  R / M", 15, 100, 100, 100)
  world.set_text_anchor(hint_e, 0.5, 0)
  world.set_visible(hint_e, false)

  -- Extend show/hide to cover the hint label.
  -- Capture the already-defined closures from panel directly so we don't
  -- pass a stray self argument.
  local base_show = panel.show
  local base_hide = panel.hide
